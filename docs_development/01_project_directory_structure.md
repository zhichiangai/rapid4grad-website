...
1. Professor Standard/Plus/Enterprise plans UI 與正式 recurring checkout。
2. Lab owner 移除成員的完整 UI/API。
3. Subscription 失效後 Professor read-only mode。
4. V2 Admin routes 與 course content management UI。
5. 正式課程 lesson 清單、影片來源與教材資料上架。
6. 正式一次性 payment provider 與 recurring billing provider。
