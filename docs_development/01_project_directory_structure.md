...
│   ├── billing/                 # V2 Professor Lab subscription UI
│   ├── consultation/            # 公開諮詢頁
│   ├── course/                  # 課程銷售頁
...
│   ├── pricing/                 # V2 Standard/Plus 月繳、年繳與免綁卡試用
...
│   ├── prompt-builder/
│   ├── quiz/
│   ├── subscriptions/           # Professor recurring provider + ECPay checksum
│   ├── stripe/
│   ├── supabase/
│   └── workspace/
├── public/
...
| `/learn` | `app/learn/page.tsx` | Server + Client player | `IMPLEMENTED` V2 local closure | 跨 workspace 共用課程播放器；RLS 只回傳當前使用者可看的 lesson metadata |
| `/pricing` | `app/pricing/page.tsx` | Server + Client | `IMPLEMENTED` V2 local closure | Professor Standard/Plus 月繳、年繳與一次 30 天免綁卡試用；價格未公告時正式 checkout disabled |
| `/billing` | `app/billing/page.tsx` | Server + Client cancel | `IMPLEMENTED` V2 local closure | Professor Lab subscription、trial、15 天 past_due grace 與唯讀狀態 |
...
V2 Task 5 已完成 Local closure：

- Standard 0–15、Plus 0–30、Enterprise 31+ 顯示與 DB 席位限制。
- 每位 Professor 一個 active owned Lab、每 Lab 一筆 current subscription。
- Standard/Plus 月繳與年繳資料模型、30 天免綁卡試用與 15 天 past_due grace。
...
- `components/course/CourseLearningExperience.tsx`：三層課程 catalog、原生 `<video>` 播放器與本人進度同步。
...
- `components/billing/ProfessorPricingClient.tsx`：Standard/Plus 月繳、年繳、試用與 ECPay form POST。
- `components/billing/CancelSubscriptionButton.tsx`：Professor 自助停止試用或後續自動續訂。
...
| `/api/billing/trial` | `app/api/billing/trial/route.ts` | POST | 一次 30 天免綁卡 Professor Lab trial RPC | `IMPLEMENTED` V2 local closure |
| `/api/billing/checkout` | `app/api/billing/checkout/route.ts` | POST | V2 Lab subscription order + ECPay recurring form POST | `IMPLEMENTED` local closure；價格／credentials 待外部設定 |
| `/api/billing/webhook/ecpay` | `app/api/billing/webhook/ecpay/route.ts` | POST | CheckMacValue 驗證 + 原子 subscription event RPC | `IMPLEMENTED` local closure；外部 webhook 待驗收 |
| `/api/billing/cancel` | `app/api/billing/cancel/route.ts` | POST | ECPay PeriodAction + service-only cancel RPC | `IMPLEMENTED` local closure；外部 ECPay 待驗收 |
| `/api/billing/portal` | `app/api/billing/portal/route.ts` | POST | Legacy endpoint；明確回覆使用 `/billing` | `DEPRECATED` |
...
| `lib/course/playback.ts` | 驗證 HTML5 MP4/WebM 播放來源與協定 | server only |
...
| `lib/subscriptions/types.ts` | Professor recurring subscription provider contract | server domain type |
| `lib/subscriptions/ecpay-signature.ts` | 綠界官方規格 CheckMacValue 產生／timing-safe 驗證 | server only |
| `lib/subscriptions/ecpay.ts` | 綠界定期定額 form POST、通知驗證與取消 | server only |
| `lib/subscriptions/provider.ts` | `SUBSCRIPTION_PROVIDER=ecpay` 選擇 | server only |
...
### 12.7 V2 Student Course Purchase — IMPLEMENTED FOUNDATION
...
Professor 與 assistant 的 `lab_basic` 課程也使用共用 `/learn`；Professor Dashboard 提供入口，但是否能看到 Lab 單元仍由有效 Lab subscription、active membership 與 RLS 動態判定。
...
app/api/course/lessons/[lessonId]/playback/route.ts
