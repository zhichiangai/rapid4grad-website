...
| `/course` | `app/course/page.tsx` | Server | `LEGACY` | 現有課程銷售入口，尚未改成 V2 買斷模型 |
...
| `/payment/success` | `app/payment/success/page.tsx` | Server | `IMPLEMENTED` | 中性付款成功頁 |
...
| `/dashboard/course` | `app/dashboard/course/page.tsx` | server Supabase access check | `LEGACY`; 尚未完全改用 V2 `course_full` |
...
- `components/course/CourseCheckoutButton.tsx`：中性 Payment Service checkout UI。
...
| `/api/payments/checkout` | `app/api/payments/checkout/route.ts` | POST | Payment Provider abstraction, orders | `IMPLEMENTED` foundation; provider incomplete |
...
- 學生 `course_full` 一次性買斷及 Lab 優惠買斷。
...
| `lib/payments/entitlements.ts` | Payment → entitlement foundation | server only；V2 需改造 |
