...
> 狀態：現行系統管理者介面與操作權限唯一規格
> 生效日期：2026-07-19
> 本文件只定義產品與安全規則，不代表 V2 Admin 程式或資料表已完成。
...
這些頁面不代表使用者、entitlement、Professor subscription、Lab seats、PDF credits 或訂單管理已完成。

## 3. V2 Admin MVP 路由

| Route | 用途 | 優先級 |
|---|---|---:|
| `/admin/users` | 查詢帳號、workspace role 與帳號狀態 | MVP |
| `/admin/entitlements` | 查詢、補發、撤銷個人課程權限 | MVP |
| `/admin/labs` | 查看 Lab、成員、方案與席位 | MVP |
| `/admin/subscriptions` | 查看 Professor subscription 與同步狀態 | MVP |
| `/admin/orders` | 查看一次性訂單與付款 | MVP |
| `/admin/pdf-credits` | 查看及受控補充 Lab PDF 額度 | MVP |
| `/admin/action-logs` | 查看敏感管理操作紀錄 | MVP |
...
- Standard／Plus 的預設 PDF 額度目前尚未定案；Task 5 只保留可設定的後端結構，不建立假額度紀錄。未來由受控 Admin 介面設定方案預設值與例外調整。
- Admin 可查看 Lab limit、reserved、used、remaining 與週期。
- Admin 可因系統失敗或客服補償補回額度，必須填寫原因。
...
V2 Baseline 應規劃 `admin_action_logs`，至少包含：
...
- Admin 是否需要人工核准 professor role，或付款成功自動啟用。
- 客服延長 subscription 的最長期限。
- PDF credit 補償上限與是否需要第二位管理者批准。
