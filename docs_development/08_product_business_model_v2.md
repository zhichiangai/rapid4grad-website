...
「研究報告 AI 指令產生器」是獨立的免費引流與研究準備工具：系統在瀏覽器端依情境拼接 Prompt，使用者自行貼到外部 AI，不消耗 RAPID 的模型 API 額度。

- 未登入訪客每個瀏覽器可匿名使用 20 次，使用 HttpOnly cookie 記錄。
- 完成 Email 驗證後可不限次使用；Google 登入帳號亦視為已驗證身分，可不限次使用。
- 此工具不以學生課程買斷、Lab membership、Professor subscription 或 Admin role 決定用量。
- Email 驗證只用於建立基本防濫用與後續聯繫的信任門檻；驗證碼寄送仍保留冷卻、錯誤次數與頻率防線。
- 平台內 PDF AI 稽核是另一個付費 Lab shared pool 功能，絕不可將兩者的額度混用。
