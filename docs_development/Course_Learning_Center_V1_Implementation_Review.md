...
- 使用既有 playback API 取得已授權的 HTTPS MP4/WebM 播放來源；瀏覽器不接觸 service credential。
...
可建立或編輯課程單元：title、slug、module、description、access level、HTML5 video source、material URL、sort order、published。網址只接受 HTTPS；影片限 MP4/WebM，provider 固定為 `html5`。本輪不提供 hard delete，發布仍由既有 `is_published` 控制。另提供 `/admin/course/preview` 靜態預覽，不會取得影片來源，也不會寫入觀看進度。
...
- Contract tests cover deterministic resume, progress persistence, module navigation, mobile curriculum, dashboard entry, Admin guard, URL/provider validation, no-delete rule and no browser service credentials.
...
- Preview deployment: 待 branch push 後填入實際 URL、Deployment ID、commit 與 READY 狀態
