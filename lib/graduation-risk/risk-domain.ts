...
export function deriveGraduationRiskSignals(input: {
  ...
  if (weeklyAge !== null && weeklyAge >= 14) signals.push({ key: "update_overdue", severity: "urgent", title: "Weekly 已超過兩週未更新", reason: "最近一次 Weekly 更新距今已達 14 天以上。", recommendation: "先填寫本週研究進度，重新建立研究節奏。", href: "/dashboard/weekly-check-in", source: "weekly" });
  else if (weeklyAge !== null && weeklyAge >= 7) signals.push({ key: "no_recent_update", severity: "attention", title: "Weekly 最近較少更新", reason: "最近一次 Weekly 更新距今已達 7 天以上。", recommendation: "更新本週研究進度，讓下一步更清楚。", href: "/dashboard/weekly-check-in", source: "weekly" });
  ...
  const noRecent = completed ? now.getTime() - new Date(completed.meeting_at).getTime() >= 21 * 86400000 : Boolean(input.activeLab && input.joinedAt && now.getTime() - new Date(input.joinedAt).getTime() >= 21 * 86400000);
...
export function deriveGraduationRiskStatus(input: { signals: GraduationRiskSignal[]; hasThesisRows: boolean; activeLab: boolean; hasResearchData?: boolean }): GraduationRiskStatus {
  if (input.signals.some((signal) => signal.severity === "urgent")) return "urgent";
  if (input.signals.length) return "attention";
  if (!input.hasThesisRows && !input.hasResearchData) return "setup_needed";
  return "stable";
}
...
export function getGraduationRiskRecommendation(status: GraduationRiskStatus) {
  if (status === "setup_needed") return { title: "先建立研究資料", reason: "目前還沒有足夠資料判斷研究節奏。你可以先設定論文進度，或建立第一筆 Personal Weekly、Meeting 或下一步。", href: "/dashboard/thesis" as const, label: "設定論文進度" };
