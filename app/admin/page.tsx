...
type ActionRow = { id: string; action_type: string; target_type: string; after_state: Json | null; created_at: string };
...
export default async function AdminHomePage() {
  ...
  const [users, labs, subscriptions, credits, actions, pastDue, suspended] = await Promise.all([
    ...
    admin.from("admin_action_logs").select("id,action_type,target_type,after_state,created_at").order("created_at", { ascending: false }).limit(5).returns<ActionRow[]>(),
  ...
  return (
    <section>
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-5"><div className="flex items-end justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-200">Recent Activity</p><h2 className="mt-2 text-xl font-semibold">最近操作</h2></div><Link href="/admin/action-logs" className="text-sm text-cyan-200 hover:text-white">查看全部</Link></div><div className="mt-4 divide-y divide-white/10">{(actions.data ?? []).map((action) => <div key={action.id} className="py-3 first:pt-0"><div className="flex justify-between gap-3"><p className="text-sm font-medium text-slate-200">{action.action_type}</p><time className="text-xs text-slate-500">{new Date(action.created_at).toLocaleDateString("zh-TW")}</time></div><p className="mt-1 text-xs text-slate-500">{action.target_type} · {sanitizeAdminSnapshot(action.after_state) ? "已完成" : "已記錄"}</p></div>)}{actions.data?.length === 0 ? <p className="py-4 text-sm text-slate-500">目前沒有操作紀錄。</p> : null}</div></section>
      </div>
