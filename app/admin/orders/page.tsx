...
export default async function AdminOrdersPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const haystack = `${profile?.full_name ?? ""} ${profile?.email ?? ""} ${product?.name ?? ""} ${product?.slug ?? ""} ${order.id} ${order.provider_order_id ?? ""}`.toLowerCase();
  ...
  const productOptions = [...products.values()].sort((left, right) => left.name.localeCompare(right.name));
  return (
    <section>
      ...
      <form method="get" className="mb-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-[1.5fr_1fr_1fr_1fr_auto]"><input name="q" defaultValue={query} placeholder="搜尋姓名、Email、商品或訂單 ID" className="min-w-0 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white outline-none placeholder:text-slate-600" /><select name="status" defaultValue={selectedStatus} className="rounded-xl border border-white/10 bg-slate-950 px-4 py-3 text-sm text-white"><option value="">全部訂單狀態</option>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select><select name="product" defaultValue={selectedProduct} className="rounded-xl border border-white/10 bg-slate-950 px-4 py-3 text-sm text-white"><option value="">全部商品</option>{productOptions.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select><select name="range" defaultValue={selectedRange} className="rounded-xl border border-white/10 bg-slate-950 px-4 py-3 text-sm text-white"><option value="">全部日期</option><option value="7">近 7 天</option><option value="30">近 30 天</option><option value="90">近 90 天</option></select><button className="rounded-xl bg-cyan-400 px-5 py-3 text-sm font-semibold text-slate-950">篩選</button></form>
