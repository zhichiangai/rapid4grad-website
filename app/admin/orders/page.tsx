...
export default async function AdminOrdersPage() {
  ...
  return (
    <section>
      <AdminPageHeader eyebrow="Operations & Revenue" title="訂單" description="唯讀查看訂單與付款摘要，不顯示 checkout 或 webhook raw payload。" />
      ...
      <div className="overflow-x-auto rounded-2xl border border-white/10 bg-white/[0.025]"><table className="w-full min-w-[1000px] text-left text-sm"><thead className="border-b border-white/10 bg-white/[0.04] text-xs uppercase tracking-[0.14em] text-slate-500"><tr><th className="px-4 py-3">建立時間</th><th className="px-4 py-3">使用者</th><th className="px-4 py-3">商品</th><th className="px-4 py-3">金額</th><th className="px-4 py-3">訂單</th><th className="px-4 py-3">付款</th><th className="px-4 py-3">Provider reference</th></tr></thead><tbody className="divide-y divide-white/10">{orders.map((order) => { const profile = profiles.get(order.user_id); const product = products.get(order.product_id); const payment = payments.get(order.id); return <tr key={order.id}><td className="px-4 py-4 text-xs text-slate-500">{new Date(order.created_at).toLocaleString("zh-TW")}</td><td className="px-4 py-4"><p className="text-slate-200">{profile?.full_name ?? "未填姓名"}</p><p className="text-xs text-slate-500">{profile?.email ?? order.user_id}</p></td><td className="px-4 py-4 text-slate-300">{product?.name ?? order.product_id}</td><td className="px-4 py-4 text-slate-200">{order.currency} {order.amount.toLocaleString("zh-TW")}</td><td className="px-4 py-4"><AdminStatusBadge status={order.status} /></td><td className="px-4 py-4"><AdminStatusBadge status={payment?.status ?? "unknown"} label={payment?.status ?? "尚無付款紀錄"} /></td><td className="px-4 py-4 font-mono text-xs text-slate-500">{order.provider}: {shorten(order.provider_order_id)}<br />payment: {shorten(payment?.provider_payment_id ?? null)}</td></tr>; })}{orders.length === 0 ? <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-400">目前沒有訂單。</td></tr> : null}</tbody></table></div>
    </section>
  );
}
