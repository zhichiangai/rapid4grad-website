...
export async function saveCourseLesson(formData: FormData) {
  ...
  const playbackId = lesson.videoProvider === "mux" ? lesson.muxPlaybackId : lesson.videoSource;
  const validVideo = lesson.videoProvider === "mux" ? (!lesson.isPublished || isValidMuxPlaybackId(playbackId)) : validUrl(playbackId, !lesson.isPublished);
