...
export default async function AdminCoursePreviewPage({ searchParams }: { searchParams: Promise<{ lessonId?: string }> }) {
  ...
  const { data: lesson } = params.lessonId ? await admin.from("course_lessons").select("id,course_id,slug,module_key,title,description,access_level,material_url,sort_order").eq("id", params.lessonId).maybeSingle() : { data: null };
