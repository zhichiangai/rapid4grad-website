  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(6,182,212,0.16),transparent_34rem),linear-gradient(180deg,#020617_0%,#0f172a_100%)] px-4 py-10 text-white">
      <section className="mx-auto w-full max-w-6xl space-y-6">
        ...
        <WeeklyCheckInStatusCard mode={mode} hasActiveLab={Boolean(activeLab)} hasHistory={updates.length > 0} />
        ...
        {mode !== "none" || activeLab ? (
          <section className="grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-6">
              ...
              <WeeklyCheckInForm currentUpdate={currentUpdate} disabled={!canWrite} />
            </div>
            ...
            <aside className="space-y-4">
              <div className="rounded-[2rem] border border-cyan-300/15 bg-cyan-400/[0.06] p-6">
                ...
                <p className="mt-3 text-sm leading-7 text-cyan-50">提交後，這份每週進度會成為你所在 Lab 的研究指導資料，供有權限的教授 / Assistant 查看。</p>
                <p className="mt-3 text-sm leading-7 text-cyan-100/70">你的私人 PDF、原始 AI 對話與私人筆記不會因此自動分享。</p>
              </div>
              {activeLab ? <div className="rounded-[2rem] border border-white/10 bg-slate-950/60 p-6"><p className="text-xs uppercase tracking-[0.2em] text-slate-500">Active Lab</p><p className="mt-3 font-semibold text-white">{activeLab.labs?.name}</p><p className="mt-2 text-sm text-slate-400">{mode === "functional" ? "可提交本週進度" : "目前為唯讀模式"}</p></div> : null}
            </aside>
          </section>
        ) : null}
