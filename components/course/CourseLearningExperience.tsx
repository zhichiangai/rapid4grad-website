...
function CourseLearningExperienceBase({ course, lessons, isAuthenticated, previewMode = false, playbackEndpoint }: { course: { title: string; description: string | null }; lessons: CourseLessonView[]; isAuthenticated: boolean; previewMode?: boolean; playbackEndpoint?: string }) {
  ...
  useEffect(() => {
    ...
    async function loadPlayback() {
      try {
        const response = await fetch(playbackEndpoint ? `${playbackEndpoint}/${encodeURIComponent(selectedLessonId)}` : `/api/course/lessons/${encodeURIComponent(selectedLessonId)}/playback`, { cache: "no-store", signal: controller.signal });
  ...
  }, [playbackEndpoint, previewMode, selectedLessonId]);
