...
type PlaybackResponse = { success?: boolean; error?: string; playback?:
  | { provider: "html5"; lessonId: string; title: string; src: string; contentType: "video/mp4" | "video/webm" }
  | { provider: "mux"; lessonId: string; title: string; playbackId: string; playbackToken: string }
};
...
function CourseLearningExperienceBase({ course, lessons, isAuthenticated, previewMode = false, previewPlaybackEnabled = false, previewMessage, playbackEndpoint }: { course: { title: string; description: string | null }; lessons: CourseLessonView[]; isAuthenticated: boolean; previewMode?: boolean; previewPlaybackEnabled?: boolean; previewMessage?: string; playbackEndpoint?: string }) {
  ...
  useEffect(() => {
    if (!selectedLessonId || (previewMode && !previewPlaybackEnabled)) {
      setIsLoading(false);
      setPlayback(null);
    ...
    async function loadPlayback() {
      try {
        const response = await fetch(playbackEndpoint ? `${playbackEndpoint}/${encodeURIComponent(selectedLessonId)}/playback` : `/api/course/lessons/${encodeURIComponent(selectedLessonId)}/playback`, { cache: "no-store", signal: controller.signal });
  ...
  const handleTimeUpdate = (event: MediaEvent) => { const second = Math.floor(event.currentTarget.currentTime); if (second - lastSyncedSecond.current < 30) return; lastSyncedSecond.current = second; void persistProgress("in_progress", second); };
  const handlePause = (event: MediaEvent) => { void persistProgress("in_progress", event.currentTarget.currentTime); };
  const handleLoadedMetadata = (event: MediaEvent) => { const resume = safeResumeSeconds(selectedLesson); const duration = event.currentTarget.duration; if (resume > 0 && Number.isFinite(duration) && resume < duration) event.currentTarget.currentTime = resume; };
...
export function CourseLearningExperience(props: React.ComponentProps<typeof CourseLearningExperienceBase>) {
  ...
  return (
    <div>
      ...
      <CourseLearningExperienceBase {...props} />
    </div>
  );
}
