...
export function ProfessorAiEntry({ contextType, labId = null, studentId = null }: Props) {
  ...
  const [lastAudio, setLastAudio] = useState<{ blob: Blob; durationSeconds: number } | null>(null);
  ...
  async function startRecording() {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        setLastAudio({ blob, durationSeconds });
        await submitVoice(blob, durationSeconds, false);
      };
      recorderRef.current = recorder; startedAtRef.current = Date.now(); setSeconds(0); setRecording(true); recorder.start();
      stopTimerRef.current = window.setTimeout(() => recorder.stop(), 120_000);
  ...
  async function submitVoice(blob: Blob, durationSeconds: number, precision: boolean) {
    const form = new FormData();
    form.append("audio", blob, "rapid-voice.webm");
    ...
    try {
      ...
      setProposal(result.proposal); setOperationId(result.operationId); setLastAudio(null);
      setQuotaWarning(Boolean(result.quotaWarning));
