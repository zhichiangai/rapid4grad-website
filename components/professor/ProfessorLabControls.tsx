...
export function ProfessorLabControls({
  ...
  const [intendedRole, setIntendedRole] = useState<LabRole>("student");
    ...
    <section className="grid gap-4 lg:grid-cols-[1fr_1.15fr]">
      ...
      >
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <label className="block text-sm text-slate-300 sm:col-span-2 lg:col-span-4">
            指定 Lab
            <select
              value={selectedLabId}
          ...
          <label className="block text-sm text-slate-300">
            ...
            <select
              value={intendedRole}
              onChange={(event) => setIntendedRole(event.target.value as LabRole)}
              className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-white outline-none transition focus:border-cyan-400"
            >
        ...
        {createdInvite ? (
          ...
          >
            ...
            <p className="mt-2 text-sm text-slate-300">
              指定角色：{createdInvite.invite.intended_role}。受邀者到
              /dashboard/lab-join 輸入此碼。到期時間：
