...
export function LabPlanningPanel({ labId, initialMilestones, initialResources, canManage, readOnlyReason }: { labId: string; initialMilestones: LabMilestone[]; initialResources: LabResource[]; canManage: boolean; readOnlyReason?: string }) {
  ...
  async function restoreResource(resource: LabResource) {
    setMessage(null);
    try {
      const result = await requestJson(`/api/professor/labs/${labId}/resources/${resource.id}`, "PATCH", { archive: false });
      if (result.resource) setResources((current) => current.map((item) => item.id === resource.id ? { ...item, archived_at: null } : item));
    } catch (error) { setMessage(error instanceof Error ? error.message : "資源還原失敗。"); }
  }
  ...
  return <section className="mt-8 grid gap-6 lg:grid-cols-2">
    ...
    <article className="rounded-3xl border border-white/10 bg-white/[0.035] p-5 sm:p-6">
      ...
      <div className="mt-5 space-y-3">{resources.length ? resources.slice(0, 8).map((resource) => <div key={resource.id} className={`rounded-2xl border p-4 ${resource.archived_at ? "border-amber-300/20 bg-amber-400/[0.04]" : "border-white/10 bg-slate-950/40"}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-white">{resource.title}</p>{resource.archived_at ? <span className="rounded-full border border-amber-300/20 px-2 py-0.5 text-[11px] text-amber-100">已封存</span> : null}</div><a href={resource.resource_url} target="_blank" rel="noreferrer" className="mt-1 block break-all text-sm text-cyan-200 hover:text-white">{resource.resource_url}</a></div>{canManage ? <button type="button" onClick={() => resource.archived_at ? restoreResource(resource) : archiveResource(resource)} className="shrink-0 rounded-xl border border-white/10 px-3 py-2 text-xs font-semibold text-slate-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70">{resource.archived_at ? "還原" : "封存"}</button> : null}</div>{resource.description ? <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-300">{resource.description}</p> : null}</div>) : <p className="rounded-2xl border border-white/10 bg-slate-950/40 p-4 text-sm text-slate-500">尚無研究資源。</p>}</div>
    </article>
